...
addServerSocket(ServerNetworkLayerTCP *layer, struct addrinfo *ai) {
    ...
    struct sockaddr_in *temp_sai = (struct sockaddr_in *)ai;
    char *s = inet_ntoa(temp_sai->sin_addr);
    UA_LOG_INFO(layer->logger, UA_LOGCATEGORY_NETWORK, "serverSocketsSize = %d  FD_SETSIZE = %d struct addrinfo ai_next: %p  IP address: %s",
                layer->serverSocketsSize, FD_SETSIZE, (void*)ai->ai_next, s);
}
...
ServerNetworkLayerTCP_start(UA_ServerNetworkLayer *nl, const UA_String *customHostname) {
  UA_initialize_architecture_network();
    ...
    for(layer->serverSocketsSize = 0; layer->serverSocketsSize < FD_SETSIZE && test != NULL;
        test = test->ai_next) {
        struct sockaddr_in *temp_sai = (struct sockaddr_in *)test;
        char *s = inet_ntoa(temp_sai->sin_addr);
        UA_LOG_INFO(layer->logger, UA_LOGCATEGORY_NETWORK,
                    "serverSocketsSize = %d  FD_SETSIZE = %d, struct addrinfo: ai_flags %d, ai_family %d, ai_socktype %d, ai_protocol %d, ai_canonname %s, ai_next: %p  IP address: %s",
                    layer->serverSocketsSize, FD_SETSIZE, test->ai_flags,       
                    test->ai_family, test->ai_socktype, test->ai_protocol,              
                    test->ai_canonname,                
                    (void*)test->ai_next, s);
    }
    */
