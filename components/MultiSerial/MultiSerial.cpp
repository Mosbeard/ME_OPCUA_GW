...
#include "esp_log.h" //wb mg include ESP-IDF logging library
...
static const char* MS_TAG = "MS_Library";  // Graf default: NULL
...
void MultiSerial::begin(unsigned long baud, unsigned long crystalHz) {
	...
	msWriteRegister(LCR, 128); 	// 128 = 1000 0000
	...
	/*
	//switch to the enhanced register set
	msWriteRegister(LCR, 191);  // 191 = 0xBF = 1011 1111
	//enable Auto-RTS and enhanced functions
	msWriteRegister(EFR, 80);  // 80 = 0x50 = 0101 0000 
	ESP_LOGD(MS_TAG, "EFR Register read: %d", msReadRegister(EFR));    // Graf default: NULL
	*/
	// switch back to the normal register bank
	msWriteRegister(LCR, 0);
	// set the word size to 8 bits
	setWordSize(8);
	ESP_LOGD(MS_TAG, "LCR Register read: %d", msReadRegister(LCR));    // Graf default: NULL
	//msWriteRegister(MCR, 2);
	//ESP_LOGD(MS_TAG, "MCR Register read: %d", msReadRegister(MCR));    // Graf default: NULL
	//msWriteRegister(IER, 1);	// 1 = 0x01 = 0000 0002 RHR interrupt enabled
	// enable the TX/RX fifos
	msWriteRegister(FCR, 7); 	// 7 = 0x07 = 0000 0111 msWriteRegister(FCR, 199): 199 = 1100 0111 sets the RX FIFO trigger level to 60 characters 3*19=57 3 MTC telegrams can be stored Graf msWriteRegister(FCR, 199)
	
	//msWriteRegister(MCR, 4);  // 4 = 0x04 = 0000 0100 enable TCR and TLR Graf default: NULL
	//msWriteRegister(TCR, 175);  // 175 = 0xAF = 0101 1111 set the RX FIFO trigger level to halt the transmission at 60 characters and to resume the transmission at 10 characters Graf default: NULL
	//msWriteRegister(TLR, 240);  // 240 = 0xF0 = 1111 0000 set the RX trigger level to 60 words for interrupt generation for 3 MTC telegrams with 3 * 19 = 57  Graf default: NULL
}
...
size_t MultiSerial::write(uint8_t val) {
	return msWriteRegister(THR, val); 
}
...
void MultiSerial::enableInterrupt(byte interruptTypes) {
	...
	ESP_LOGI(MS_TAG, "IER Register read: %d", msReadRegister(IER));      // Graf default: NULL
}
...
//Graf start wrapper function for msReadRegister
byte MultiSerial::ReadRegister(byte _reg)
{
	return msReadRegister(_reg);
}

byte MultiSerial::WriteRegister(byte _reg, byte _val)
{
	return msWriteRegister(_reg, _val);
}

// Description: reset RX fifo
// Returns: nothing
void MultiSerial::resetRXfifo() {
	byte regValue;
	regValue = msReadRegister(FCR);
	regValue = regValue | 0x02;
	msWriteRegister(FCR, regValue);
}

// Description: rxFIFOenable
// Returns: nothing
void MultiSerial::rxFIFOenable() {
	byte regValue;
	regValue = msReadRegister(FCR);
	regValue = regValue | 0x01;
	msWriteRegister(FCR, regValue);
}
// Description: rxFIFOdisable
// Returns: nothing
void MultiSerial::rxFIFOdisable() {
	byte regValue;
	regValue = msReadRegister(FCR);
	regValue = ~0x01 & regValue;
	msWriteRegister(FCR, regValue);	
}
//Graf end
...
byte MultiSerial::msWriteRegister(byte reg, byte val) {
	int err_val = 0;
...
// Graf start
MultiSerial* SC16IS752_CH_A = new MultiSerial(SC16IS752_I2C_AD, CHANNEL_A);
MultiSerial* SC16IS752_CH_B = new MultiSerial(SC16IS752_I2C_AD, CHANNEL_B);
// Graf end